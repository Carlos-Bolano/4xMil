...
export function ResultCard({ title, amount, icon, variant = "default", className }: ResultCardProps) {
  ...
  const containerStyle = isPrimary
    ? "bg-white dark:bg-primary/10 border-primary/20 p-4"
  ...
  return (
    <View
      className={`flex-row items-center justify-between rounded-2xl border shadow-sm ${containerStyle} ${className}`}
    >
      <View className="flex-row items-center gap-3">
        {icon}
        <Text className={`font-medium text-slate-900 dark:text-slate-100 ${isPrimary ? "font-bold" : ""}`}>
          {title}
        </Text>
      </View>
